...
def tw_search_json(query, cnt=5):
    ...
    for tweet in tweepy.Cursor(api.search, q=query, count=cnt).items():
        ...
        dTwt['lang'] = tweet.lang           #####
        dTwt['coord']= tweet.coordinates
...
def tw_search(api):
    ...
    csvFile = open('Tourism.csv','a')
    #Use csv Writer
    csvWriter = csv.writer(csvFile)
    csvWriter.writerow(["created", "username", "text", "tweet_id", "location", "retwc", "hashtag","hashtag1", "hashtag2", "hashtag3", "hashtag4","followers", "friends", "author_id", "source","lang", "coord"])################
    ...
    for tweet in tweepy.Cursor(api.search,
        ...
        try:
            ...
            hashtag1 = tweet.entities[u'hashtags'][1][u'text'] #hashtags used
            hashtag2 = tweet.entities[u'hashtags'][2][u'text'] #hashtags used
            hashtag3 = tweet.entities[u'hashtags'][3][u'text'] #hashtags used
            hashtag4 = tweet.entities[u'hashtags'][4][u'text'] #hashtags used

            #hashtag =  [hashtag['text'] for hashtag in tweet['entities']['hashtags']]
           # hashtag = tweet.entities.get(['hashtags'])#hashtags used
            #hashtag = tweet.entities[u'hashtags'][1][1][u'text'] #hashtags used
         #  hashtag = tweets['Hashtags'][1][1]['text']
        except:
            hashtag = "None"
            hashtag1= "None"
            hashtag2= "None"
        ...
        # Dongho 03/28/16
        csvWriter.writerow([created, username, text, tweet_id, loc, retwc, hashtag, hashtag1,hashtag2, hashtag3, hashtag4, followers, friends, author_id, source, lang, coord])##################################
        counter = counter +1
        if (counter == c):
            break
